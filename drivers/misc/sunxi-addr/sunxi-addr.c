...
#define ADDR_MGMT_DBG(fmt, arg...) printk(KERN_DEBUG "[ADDR_MGMT] %s: " fmt "\n",\
				__func__, ## arg)
#define ADDR_MGMT_ERR(fmt, arg...) printk(KERN_ERR "[ADDR_MGMT] %s: " fmt "\n",\
				__func__, ## arg)
...
static struct addr_mgmt_info info[] = {
...
static struct addr_mgmt_info *addr_find_by_name(char *name)
{
	int i = 0;
...
{
	struct addr_mgmt_info *t;

	t = addr_find_by_name(name);
	if (t == NULL) {
		ADDR_MGMT_ERR("can't find addr named: %s", name);
		return -1;
	}

	if (IS_TYPE_INVALID(t->type_cur)) {
		ADDR_MGMT_ERR("addr type invalid");
		return -1;
	}

	if (addr_parse(ADDR_FMT_VAL, t->addr, t->flag)) {
		ADDR_MGMT_ERR("addr parse fail(%s)", t->addr);
		return -1;
	}
...
static int set_addr_by_name(int type, int fmt, const char *addr, char *name)
{
	...
	if (addr_parse(fmt, addr, t->flag)) {
		ADDR_MGMT_ERR("addr parse fail(%s)", addr);
		return -1;
	}
			...
			int idx, int type, char *mac, char *name)
{
	...
	case TYPE_RAND:
		...
		break;
	default:
		ADDR_MGMT_ERR("unsupport type: %d", type);
		return -1;
	}
	return 0;
}
...
static int addr_init(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(info); i++) {
		...
		if (of_property_read_u32(np, match, &type)) {
			ADDR_MGMT_DBG("Failed to get type_def_%s, use default: %d",
...
static const struct of_device_id addr_mgmt_ids[] = {
	{ .compatible = "allwinner,sunxi-addr-mgmt" },
	{ /* Sentinel */ }
};

static int addr_mgmt_probe(struct platform_device *pdev)
{
	int status;

	ADDR_MGMT_DBG("module version: %s", MODULE_CUR_VERSION);
	status = class_register(&addr_class);
	if (status < 0) {
		ADDR_MGMT_ERR("class register error, status: %d.", status);
		return -1;
	}

	if (addr_init(pdev)) {
		ADDR_MGMT_ERR("failed to init addr.");
		class_unregister(&addr_class);
		return -1;
	}
	ADDR_MGMT_DBG("success.");
	return 0;
}

static int addr_mgmt_remove(struct platform_device *pdev)
{
	class_unregister(&addr_class);
	return 0;
}

static struct platform_driver addr_mgmt_driver = {
	.probe  = addr_mgmt_probe,
	.remove = addr_mgmt_remove,
	.driver = {
		.owner = THIS_MODULE,
		.name  = "sunxi-addr-mgmt",
		.of_match_table = addr_mgmt_ids,
	},
};

module_platform_driver_probe(addr_mgmt_driver, addr_mgmt_probe);
